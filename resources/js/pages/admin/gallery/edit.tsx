...
export default function AdminGalleryEdit({ photo }: GalleryPhotoProps) {
    ...
    function handleVideoChange(e: React.ChangeEvent<HTMLInputElement>) {
        ...
        if (file) {
            ...
            if (file.size > maxSize) {
                alert(`Ukuran video terlalu besar! Max 100MB. Video Anda: ${(file.size / (1024 * 1024)).toFixed(2)}MB`);
                e.target.value = '';
                return;
            }
    ...
    const currentVideoSrc = videoPreview || (!data.remove_video ? photo.video_path : null);
    const hasVideo = Boolean(currentVideoSrc) || Boolean(data.video_url && !data.remove_video);
                ...
                <header className="flex flex-col justify-between gap-4 border-b border-sidebar-border/70 pb-6 md:flex-row md:items-end">
                    <div>
                        ...
                        <h1 className="mt-2 text-3xl font-bold tracking-tight text-foreground">
                            Sunting {data.media_type === 'photo' ? 'Foto' : 'Video'} Galeri
                        </h1>
                    </div>
                </header>
                ...
                <form onSubmit={handleSubmit} className="max-w-4xl space-y-6">
                    ...
                    <div className="space-y-5 rounded-xl border border-sidebar-border/70 bg-background p-6 shadow-xs">
                        ...
                        <div className="flex items-center gap-4 rounded-xl border border-sidebar-border/70 bg-muted/40 p-4">
                            <div className="size-20 shrink-0 overflow-hidden rounded-lg border border-sidebar-border bg-muted">
                                {photo.media_type === 'photo' && photo.image_path ? (
                                    <img
                                    ...
                                    <button
                                        type="button"
                                        onClick={() => currentVideoSrc && setShowVideoModal(true)}
                                    ...
                                    >
                                        ...
                                        <div className="absolute inset-0 flex items-center justify-center bg-black/40 transition-colors group-hover:bg-black/50">
                                            <Play className="size-6 text-white drop-shadow" fill="white" />
                                        </div>
                                    </button>
                                ) : null}
                            </div>
                            <div className="min-w-0 flex-1">
                                <p className="text-xs font-bold text-foreground">
                                    Pratinjau {photo.media_type === 'photo' ? 'Foto' : 'Video'} Saat Ini
                                </p>
                                {photo.media_type === 'photo' ? (
                                    ...
                                    <div className="mt-1 flex items-center gap-2">
                                        <p className="line-clamp-1 text-xs text-muted-foreground">
                                            {videoPreview ? 'Video baru dipilih' : getFileName(photo.video_path) || 'URL Video'}
                                        </p>
                                        <button
                                            type="button"
                                    ...
                                    <p className="mt-1 text-xs text-muted-foreground">Belum ada video</p>
                                )}
                            </div>
                            {/* Click to play hint for video */}
                            {photo.media_type === 'video' && currentVideoSrc && (
                                <button
                                    type="button"
                            ...
                            <div className="flex gap-4">
                                <label className="flex items-center gap-2 cursor-pointer">
                                    <input
                                        ...
                                        onChange={() => setData('media_type', 'photo')}
                                ...
                                <div className="grid gap-5 border-t border-sidebar-border/70 pt-2 sm:grid-cols-2">
                                    <div>
                                        ...
                                        <p className="mt-1 text-xs text-muted-foreground">
                                            Format: MP4, WebM, AVI, MOV (max 100MB)
                                        </p>
                                        {errors.video && (
                                            <p className="mt-1 text-xs text-red-600">{errors.video}</p>
                                        )}
                                    </div>
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        >
                            ...
                            <span>
                                {processing ? 'Menyimpan... Harap tunggu' : `Perbarui ${data.media_type === 'photo' ? 'Foto' : 'Video'}`}
                            </span>
                        </button>
                        <Link
                            href={galleryIndex()}
                        ...
                        {processing && (
                            <div className="text-xs text-muted-foreground">
                                <div className="flex items-center gap-2">
                                    ...
                                    <span>Mengupload file... Jangan tutup halaman</span>
                                </div>
                            </div>
                        )}
                    </div>
