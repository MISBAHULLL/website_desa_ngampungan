...
export default function AdminVillageOfficialsIndex({
    officials,
    filters,
}: Props) {
    ...
    function handleSearchSubmit(e: React.FormEvent) {
        ...
        router.get(
            officialIndex.url(),
            { search: searchQuery, group: filters.group },
            { preserveState: true },
        );
    }
    ...
    function handleDelete(id: number, name: string) {
        if (confirm(`Apakah Anda yakin ingin menghapus perangkat "${name}"?`)) {
            setDeletingId(id);
            router.delete(officialDestroy.url(id), {
        ...
        <>
            ...
            <div className="flex flex-1 flex-col gap-6 p-4 md:p-6">
                <header className="flex flex-col justify-between gap-4 border-b border-sidebar-border/70 pb-6 md:flex-row md:items-end">
                    <div>
                        ...
                        <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
                            Tambah, sunting, dan atur data perangkat desa
                            termasuk foto, jabatan, dan profil detail modal.
                        </p>
                    </div>
                    <Link
                        href={officialCreate()}
                ...
                {officials.data.length > 0 ? (
                                                ...
                                                <div className="flex items-center gap-3">
                                                    {item.photo_url ? (
                                                        <img
                                                            src={item.photo_url}
                                                            alt={item.name}
                                                            className="size-10 rounded-lg border border-sidebar-border/70 object-cover"
                                                        />
                                                    ) : (
                                                        <span className="flex size-10 items-center justify-center rounded-lg border border-emerald-200 bg-emerald-50 text-xs font-extrabold text-emerald-800 dark:border-emerald-800 dark:bg-emerald-950/60 dark:text-emerald-300">
                                                            {item.initials}
                                                        </span>
                                                    )}
                    ...
                    <div className="flex min-h-72 flex-col items-center justify-center rounded-xl border border-dashed border-sidebar-border bg-muted/20 p-8 text-center">
                        ...
                        <p className="mt-2 max-w-md text-sm leading-6 text-muted-foreground">
                            Belum ada data perangkat desa yang terdaftar atau
                ...
                {officials.last_page > 1 && (
                    <nav
                        aria-label="Pagination perangkat admin"
